...
class ProductExtractor:
    """Extract product data from e-commerce platforms using various methods"""
    ...
    def extract_shopify_products(self, html_content: str, url: str = "") -> List[Product]:
        """
        Extract product data from Shopify pages using the fast path approach.
        
        This method looks for Shopify's inline analytics blob that contains
        product data in a structured format, avoiding the need to parse HTML.
        ...
        Returns:
            List of Product model instances with normalized fields
            
        Raises:
            ProductExtractionError: If extraction fails
        """
        try:
            logger.info(f"Extracting Shopify products from {'URL: ' + url if url else 'HTML content'}")
            ...
            logger.info(f"Successfully extracted {len(products)} products from Shopify page")
            return products
    ...
    def _extract_product_variants(self, script_content: str) -> List[Dict[str, Any]]:
        ...
        Args:
            script_content: The script content containing product data
    ...
    def _create_shopify_product(self, variant_data: Dict[str, Any], source_url: str = "") -> Optional[Product]:
        ...
        Args:
            variant_data: Raw product variant data from Shopify
        ...
        try:
            ...
            if product.title or product.product_id:
                return product
            else:
            ...
            logger.error(f"Error creating product from variant data: {e}")
            return None
